...
public class AddDoctorServlet extends HttpServlet {
    private static final Logger logger = LoggerFactory.getLogger(AddDoctorServlet.class);

    private UserDao doctorDao;

    public void init() {
        doctorDao = new UserDao();
    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        logger.info("Execute doPost method in AddDoctorServlet");
        ...
        int roleId = 3; String createdBy = "admin";
        ...
        !validFirstname || !validLastname || !validPhonenumber ||!validEmail){
            ...
            doctorDao.insert(user);
            request.getSession().setAttribute("successInsert", "Doctor was successfully added! ");
            //всё верно, открываю список докторов
             response.sendRedirect(Links.DOCTOR_LIST_JSP);
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        logger.info("Execute doGet method in AddDoctorServlet");
        request.getRequestDispatcher(Links.DOCTOR_JSP).forward(request, response);
